...
export function FeatureReviewCard({ data, onProceed, onStop, readOnly }: FeatureReviewCardProps) {
  ...
  return (
    <div className="w-full max-w-2xl animate-spring-in">
      <div className="rounded-xl border border-white/[0.08] bg-[#161616] overflow-hidden shadow-elevation-1">
        {/* Header */}
        <div className="px-5 py-3.5 border-b border-white/[0.06] flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-white/80">{data.projectMeta?.name || "Project Plan"}</h3>
            <p className="text-[11px] text-white/30 mt-0.5">{data.projectMeta?.description}</p>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          {/* Features as stepper */}
          <div>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-white/25 mb-2 block">Features</span>
          ...
          {/* Architecture */}
          <div>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-white/25 mb-1.5 block">Architecture</span>
            <div className="flex items-start gap-2.5 px-3.5 py-2.5 rounded-lg bg-white/[0.03] border border-white/[0.06]">
              ...
              <span className="text-xs text-white/40 leading-relaxed">{data.architecture}</span>
            </div>
          </div>
                ...
                <div className="mt-2 space-y-1.5 pl-1">
                  {data.frontendTasks.map((t, i) => (
                    <div key={i} className="px-3 py-2 rounded-lg bg-emerald-500/[0.03] border border-emerald-500/10">
                      ...
                      <p className="text-[11px] text-white/30 mt-0.5">{t.details}</p>
                    </div>
                  ))}
                </div>
              </Collapse>
          ...
          {data.notes && (
            <div>
              <span className="text-[10px] font-semibold uppercase tracking-widest text-white/25">Notes</span>
              <p className="text-xs text-white/35 mt-1 leading-relaxed">{data.notes}</p>
            </div>
          )}
        </div>
        ...
        {!isCompleted && needsDb && (
          <div className="flex items-center gap-3 px-5 py-3 border-t border-white/[0.06]">
            ...
            <div className="flex-1">
              ...
              <p className="text-[10px] text-white/20">Free 250MB SQLite database for preview & testing</p>
            </div>
            <button
        ...
        {!isCompleted && (
          <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-white/[0.06]">
            ...
            <Button size="sm" onClick={handleProceed} className="text-xs h-8 px-4 bg-[#D4AF37] hover:bg-[#E0C050] text-black border-0">
