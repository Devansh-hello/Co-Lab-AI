...
export async function handleQAComplete(
    ...
    ctx.pipeline.qaAnswers = answers;
    ctx.pipeline.phase = 'planning';
    ctx.pipeline.messageDoc.qaAnswers = answers;
    await ctx.pipeline.messageDoc.save();

    /* Build enriched message for the orchestrator (includes Q&A context) */
    const originalMessage = ctx.pipeline.messageDoc.userMessage;
    ...
    (ctx.pipeline.messageDoc as any)._enrichedMessage = `${originalMessage}${qaContext}`;
